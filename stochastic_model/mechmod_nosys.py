...
from functions_plus_plus import ccdA_conc
from functions_plus_plus import simulate_markov_chain
from functions_plus_plus import ccda_odes
from functions_plus_plus import ccdr_odes
from functions_plus_plus import TF_conc
from functions_plus_plus import stoc_mod
from functions_plus_plus import stoc_mod_plus_plus
import math as mt
import scipy as sp
...
initial_conditionsTF = [0, (3*10**-15), (2.5*10**-15), (9*10**-15), (1*10**-15), 0] # R_MRNA, R, R_2, R_4, R_8, TF

initial_conditionsA = [0, 0, 1500, 0, 0, 0] # A_MRNA, B_MRNA, A, B, C, C_2

steps = range(100000) #how long the simulation runs for
...
# Save results
output_file = f"outputs/simnocys_{job_id}.npz"
np.savez(output_file, times=times, A=A_traj, B=B_traj, A_MRNA=A_MRNA_traj, B_MRNA=B_MRNA_traj, C=C_traj, C_2=C_2_traj, TF=TF_traj, states=states)

data = np.load(f'outputs/simnocys_{job_id}.npz')
...
fig.tight_layout()
plt.savefig(f'output_figures/simnocys_{job_id}_graph.png')
plt.close()
