...
def ccda_odes(t, y, vk_BMRNA):
    ...
    k_AMRNA = 0.044016 #Approximation - CcdA mRNA formation rate real value 0.02751  0.044016 0.033012
...
#0.01834
    ...
    k_BMRNA = vk_BMRNA #Approximation - CcdB mRNA formation rate - has arg set to the variable transcription rate based on bound/unbound state
...
def ccdr_odes(t, x):
    ...
    k_RMRNA = (6.97*10**-12) #Estimation based on tuned CcdA param. - CcdR mRNA formation rate
    k_degM = (0.0203) #From Gelens - mRNA degradation rate
    
    #CcdR values
    
    k_Rtrans = (0.09) #Estimation - translation rate of CcdA
    k_Rdeg = (0.00115524) #Estimation based on Gelens - degradation rate of Ccdr
    
    #Dimer values
    
    k_d2 = (8.4*10**-18) #  MD modeling Estimation - based on Gao saying they only found tetramer naturally therefore both must have low K_d
    k_2f = 1*10**-3 #Tuning paramater to tune dimerization
    k_2u = 1*10**-3 #Tuning paramater to tune dimer use
    
    #Tetramer values
    
    k_d4 = (8.9*10**-17) #  MD modeling Estimation - based on Gao saying they only found tetramer naturally therefore both must have low K_d 1*10**-17
    k_4f = 1*10**-3 #Tuning paramater to tune tetramerization
    k_4u = 1*10**-3 #Tuning paramater to tune tetramer use
    
    #Octamer values
    
    k_d8 = (7.9*10**-16) # Approximation -  MD modeling Based on Gao 
    k_8f = 1*10**-3 #Tuning paramater to tune octamerization
    k_8u = 1*10**-3 #Tuning paramater to tune octamer use
    
    #Transcription factor values
    
    k_dTF = 7.7*10**-2 #(1.28*10**-9) (1.725*10**-3)# Approximation - MD modeling Based on Gao
    k_TFf = 1*10**-3 #Tuning paramater to tune cysteine binding to the octamer
    k_degTF = (1*10**-3)
    
    #Cysteine concentration
    
    #O = 1.5*10**-4
    O = 3.3*10**-3 # Constant Cysteine for Steady State testing
    #O = 2*10**-2
    
    #Differential equations
    
    dR_MRNA = k_RMRNA-k_degM*R_MRNA # CcdR mRNA formation
    dR = k_Rtrans*R_MRNA-k_Rdeg*R-((2*k_2f*R**3)/(k_d2**2+R**2)) # CcdR formation
    dR_2 = ((k_2f*R**3)/(k_d2**2+R**2))-((k_2u*2*R_2**3)/(k_d4**2+R_2**2)) # Dimer formation
    dR_4 = ((k_4f*R_2**3)/(k_d4**2+R_2**2))-((k_4u*2*R_4**3)/(k_d8**2+R_4**2)) # Tetramer formation
    dR_8 = ((k_8f*R_4**3)/(k_d8**2+R_4**2))-((k_8u*R_8*(O**8))/(k_dTF**8+O**8))#  Octamer formation
    dTF = (k_TFf*(R_8*(O**8))/(k_dTF**8+O**8))# Transcription factor formation
          
    return [dR_MRNA, dR, dR_2, dR_4, dR_8, dTF]

def ccdr_odes_plus(t, x, cyst):
    R_MRNA, R, R_2, R_4, R_8, TF=x
    ...
    k_RMRNA = (1*10**-5) #Estimation based on tuned CcdA param. - CcdR mRNA formation rate
    k_degM = (0.00203) #From Gelens - mRNA degradation rate
    ...
    k_d4 = (8.9*10**-9) #  MD modeling Estimation - based on Gao saying they only found tetramer naturally therefore both must have low K_d 1*10**-17
    ...
    k_dTF = (7.7*10**-2) #1*10^-15 Approximation - MD modeling Based on Gao
    k_TFf = 1 #Tuning paramater to tune cysteine binding to the octamer
    
    #Cysteine concentration
    
    O = cyst # updated constantly from the dFBA 
...
def TF_conc(initial_conditionsTF, steps, dt):
    ...
    for step in steps:
        ...
        sol = solve_ivp(ccdr_odes, [t_start, t_end], x, method='BDF', t_eval=([t_end]), atol=1*10**-16)
...
def TF_conc_plus_plus(initial_conditionsTF, steps, dt, cys_all):
    ...
    for cys in cys_all:
        ...
        #sets sol to the outputs of the solved ccdr_odes function at the given time point
        sol = solve_ivp(ccdr_odes_plus, [t_start, t_end], x, method='BDF', args=(cyst,), t_eval=([t_end]), atol=1*10**-16)
...
def ccdA_conc(initial_conditionsA, start_state, steps, dt, transition_matrix):
    ...
    for state in history:
        k_BMRNA = 0.0339 if state=='unbound' else 0.3846 #0.3846 0.5055 - when bound changes the expression rate of CcdB to on
...
#Bigger daddy function that puts everything together
def stoc_mod_plus_plus(initial_conditionsA, initial_conditionsTF, start_state, steps, dt):
    translist = []
    
    #Creates the list over which the function iterates
    ...
    TF_arr = TF_conc_plus_plus(initial_conditionsTF, steps, dt, cys_all)[6]
    print(TF_arr[-10:-1])
    for TF in TF_arr:
        k_on_TFP = 10000000000
        k_off_TFP = 0.001
