...
def ccda_odes(t, y, vk_BMRNA):
    ...
    k_AMRNA = 0.033012 #Approximation - CcdA mRNA formation rate real value 0.02751  0.044016
...
def ccdr_odes(t, x, cyst):
    ...
    k_RMRNA = (1*10**-14) #Estimation based on tuned CcdA param. - CcdR mRNA formation rate
    ...
    k_d2 = (8.4*10**-18) # Estimation - based on Gao saying they only found tetramer naturally therefore both must have low K_d
    k_2f = 1 #Tuning paramater to tune dimerization
    k_2u = 1 #Tuning paramater to tune dimer use
    
    #Tetramer values
    
    k_d4 = (8.9*10**-9) # Estimation - based on Gao saying they only found tetramer naturally therefore both must have low K_d 1*10**-17
    k_4f = 1 #Tuning paramater to tune tetramerization
    k_4u = 1#Tuning paramater to tune tetramer use
    
    #Octamer values
    
    k_d8 = (7.9*10**-16) # Approximation - Based on Gao 
    k_8f = 1 #Tuning paramater to tune octamerization
    k_8u = 1 #Tuning paramater to tune octamer use
    
    #Transcription factor values
    
    k_dTF = (1*10**-15) # Approximation - Based on Gao 
...
def TF_conc(initial_conditionsTF, steps, dt):
    ...
    for step in steps:
        ...
        sol = solve_ivp(ccdr_odes, [t_start, t_end], x, method='BDF', args=(cyst), t_eval=([t_end]))
