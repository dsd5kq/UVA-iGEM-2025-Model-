...
#0.01834
    ...
    dB = k_Btrans*B_MRNA-k_Bdeg*B-k_bind*A*B+k_Cdeg*C #CcdB formation
    dA = k_Atrans*A_MRNA-k_Adeg*A-k_bind*A*B+2*F*k_Adeg*C_2+F*k_Adeg*C+k_Cdeg*C #CcdA formation
...
def ccdr_odes(t, x):
    ...
    k_d2 = (1*10**-17) # complete guess - based on Gao saying they only found tetramer naturally therefore both must have low K_d
    k_2f = 1 #Tuning paramater to tune dimerization
    k_2u = 1 #Tuning paramater to tune dimer use
    
    #Tetramer values
    
    k_d4 = (1*10**-17) # complete guess - based on Gao saying they only found tetramer naturally therefore both must have low K_d
    k_4f = 1 #Tuning paramater to tune tetramerization
    k_4u = 1#Tuning paramater to tune tetramer use
    
    #Octamer values
    
    k_d8 = (1*10**-11) # guess - Based on Gao 
    k_8f = 1 #Tuning paramater to tune octamerization
    k_8u = 1 #Tuning paramater to tune octamer use
    
    #Transcription factor values
    
    k_dTF = (1*10**-11) # guess - Based on Gao 
    k_TFf = 1 #Tuning paramater to tune cysteine binding to the octamer
    
    #Cysteine concentration
    
    O = (1*10**-4) # guess - Based on cytotoxic concentration of cysteine taken one order of magnitude lower
    
    #Differential equations
    
    dR_MRNA = k_RMRNA-k_degM*R_MRNA # CcdR mRNA formation
    dR = k_Rtrans*R_MRNA-k_Rdeg*R-((4*k_2f*R**3)/(k_d2+R**2)) # CcdR formation
    ...
    dR_8 = ((k_8f*R_4**3)/(k_d4+R_4**2))-((k_8u*(R_8*O)**2)/(k_dTF+R_8*O**2)) # Octamer formation
    dTF = ((k_TFf*(R_8*O)**2)/(k_dTF+R_8*O**2)) # Transcription factor formation
          
    return [dR_MRNA, dR, dR_2, dR_4, dR_8, dTF]
          
def TF_conc(inital_conditionsTF, steps, dt):
    
    x = inital_conditionsTF #sets the intial conditions
...
def stoc_mod(initial_conditionsA, inital_conditionsTF, start_state, steps, dt, transition_matrix)