...
def cys_accumulation(flux_dict, initial_biomass, dt): 
    flux = flux_dict.get('CYSSINK', 0.0)
    delta_cys = flux * initial_biomass * dt
    return delta_cys 
...
def dfba_simulation(model, steps, dt, media_file, column_name, initial_conc_file): 
    ...
    model = cysteine_constraint(model, alpha=0.09)
    for step in range(steps):
        ...
        delta_cys = cys_accumulation(flux_dict, initial_biomass, dt)
        cys_conc += delta_cys 
        intra_cys_mmol.append(cys_conc) 
        ...
        t_total += dt
        #print('flux:', flux_dict) 
        #print('conc:', conc_dict) 
    ...
    results_df = pd.DataFrame({ 
        ...
        'intra_cys_rate': intra_cys_rate, 
        'intra_cys_conc_mmol': intra_cys_mmol,
