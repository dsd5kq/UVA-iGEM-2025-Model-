...
def update_biomass(initial_biomass, mu, dt_hr):
    #converting from g/L to gDW/L 
    initial_biomass = initial_biomass * np.exp(mu * dt_hr)
...
def dfba_simulation(model, steps, dt, media_file, column_name, initial_conc_file): 
    ...
    model = cysteine_constraint(model, alpha=0.07)
    for step in range(steps):
        ...
        initial_biomass = update_biomass(initial_biomass, mu, dt_hr)
